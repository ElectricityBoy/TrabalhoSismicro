...
{   
	 ...
	 DDR_EN = 1;                //Pines de control como salida
	 DDR_RS = 1;
	 DDR_D4 = 1;                //Pines de datos como salida
	 DDR_D5 = 1;
	 DDR_D6 = 1;
	 DDR_D7 = 1;
...
void LCD_cmd(uint8_t dato)
{
	...
	if (dato == LCD_CLEAR || dato == LCD_HOME)
		_delay_ms(2);		//tiempo mínimo 1.7ms
	else
		_delay_us(50);		//tiempo min 40 us
}
...
void LCD_dato(char dato)
{
    ...
    LCD_write_nible(dato>>4);
    LCD_write_nible(dato & 0x0F);
	_delay_us(50);			//tiempo mínimo 40us
}
